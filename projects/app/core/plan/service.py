...
from app.core.plan.errors import PlanNotFoundError
...
class PlanService:
    ...
    async def patch_plan(self, account_id: UUID, plan_id: UUID, patch: Patch) -> None:
        async with self.uow_factory() as uow:
            plan = await uow.plans.get(plan_id, account_id)
            if plan is None:
                raise PlanNotFoundError
            plan.content.patch(patch)
            await uow.plans.save(plan)
