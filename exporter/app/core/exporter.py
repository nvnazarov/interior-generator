...
class Exporter:
    ...
    async def export_project_pdf(self, project_id: str, account_id: str) -> BytesIO:
        project = await self._projects.find_project_by_id(account_id, project_id)
        if project is None:
            raise ProjectNotFoundError
        buffer = BytesIO()
        renderer = PDFRenderer(buffer)
        renderer.draw_project(project)
        async for plan in self._projects.iter_plans_in_project(account_id, project_id):
            await renderer.draw_plan(project, plan, self._catalog)
        renderer.close()
        buffer.seek(0)
        return buffer
